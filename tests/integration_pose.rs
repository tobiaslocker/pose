...
fn test_parse_pose_integration() {
    ...
    // Manually create the nested Availability struct
    let availability = Availability::create(&mut builder, &AvailabilityArgs {
        visibility: 0.9,
        presence: 0.95,
    });

    // Create Landmark objects (with and without availability)
    let l1 = Landmark::create(&mut builder, &LandmarkArgs {
        x: 1.0,
        y: 2.0,
        z: 3.0,
    ...
    let l2 = Landmark::create(&mut builder, &LandmarkArgs {
        x: 4.0,
        y: 5.0,
        z: 6.0,
        availability: None,
    ...
    let pose_result = PoseDetectionResult::create(&mut builder, &PoseDetectionResultArgs {
        landmarks: Some(landmarks_vec),
    });

    let detection_msg = DetectionMessage::create(&mut builder, &DetectionMessageArgs {
