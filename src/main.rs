...
fn score_pose_similarity(
    ...
    let Ok((live_frame, mut score)) = live_query.single_mut() else {
        return;
    };
    let Ok(ref_frame) = ref_query.single() else {
        return;
    };
    ...
    let Some(reference) = &ref_frame.0 else {
        return;
    };

    let normalize = |lms: &[Landmark]| {
        let mean_x = lms.iter().map(|l| l.x).sum::<f32>() / lms.len() as f32;
        let mean_y = lms.iter().map(|l| l.y).sum::<f32>() / lms.len() as f32;
        let scale = lms
            .iter()
            .map(|l| ((l.x - mean_x).powi(2) + (l.y - mean_y).powi(2)).sqrt())
            .sum::<f32>()
            / lms.len() as f32;

        lms.iter()
            .map(|l| ((l.x - mean_x) / scale, (l.y - mean_y) / scale))
            .collect::<Vec<_>>()
    };

    let norm_live = normalize(&live.landmarks);
    let norm_ref = normalize(&reference.landmarks);

    let mut total_dist = 0.0;
    let mut count = 0;

    for (a, b) in norm_live.iter().zip(norm_ref.iter()) {
        let dx = a.0 - b.0;
        let dy = a.1 - b.1;
    ...
    if count > 0 {
        let avg = total_dist / count as f32;
        score.0 = (1.0 - avg).clamp(0.0, 1.0) * 100.0;
    }
}
...
fn file_stream(
    ...
    let Some(instance) = audio_instances.get(&audio_handle.0) else {
        //println!("Audio instance not available yet.");
        return;
    };
